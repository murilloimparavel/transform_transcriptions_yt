...
def process_single_document(source, prompt_type, output_language, doc_number=None, total_docs=None, consolidate=False):
    ...
    try:
        ...
        if prompt_type == "framework":
            ...
            # Move arquivo gerado (prd_processor já salva no local certo, mas verificamos redundância)
            # O process_prd_framework já retorna o path correto
            
            cprint(f"\n✅ PRD BMAD completo gerado!", "green", attrs=["bold"])
            # cprint(f"   Arquivo: {output_path}", "white") # path já impresso pelo processador
...
def main():
    ...
    print("[4] PRD Completo (BMAD) - Especificação Técnica para Desenvolvimento")
